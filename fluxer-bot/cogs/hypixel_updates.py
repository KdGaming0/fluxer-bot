...
def _parse_post_content(html: str) -> dict:
    ...
    # ── Strip dynamic / meta content that changes between scrapes ──
    # Edit notices, signatures, reactions, share buttons, attribution
    for selector in [
        ".message-lastEdit", ".lastEdit", ".editDate",
        ".message-signature", ".userSignature", ".signature",
        ".messageMeta", ".message-attribution", ".shareButtons",
        ".reactionsBar", ".js-reactionList", ".reactionSummary",
        ".message-actionBar", ".actionBar-set",
        ".block-outer--after", ".block-outer--before",
        "[data-xf-init='lightbox']", ".lbContainer",
        ".bbImageWrapper", ".js-lbImage",
    ]:
        for el in post_body.select(selector):
            el.decompose()

    # Remove inline styles / scripts (sometimes injected by XenForo)
    ...
    for tag in post_body.find_all(["blockquote", "img", "figure"]):
        tag.decompose()
    for tag in post_body.find_all(class_=re.compile(r"bbCodeSpoiler-content|js-spoilerTarget")):
        tag.decompose()

    # Remove any empty paragraphs that may be left after decomposing
    for empty in post_body.find_all("p"):
...
class HypixelUpdatesCog(fluxer.Cog):
    ...
    async def _seed_guild(self, guild_id: int) -> None:
        ...
        for source_key, source_cfg in SOURCES.items():
            ...
            # Seed if empty; also re-seed if every entry has an empty hash,
            # which happens after a fresh enable or a corrupted settings reset.
            if source_seen and any(v.get("hash") for v in source_seen.values()):
                continue
    ...
    async def _check_guild(self, guild_id: int) -> None:
        ...
        for source_key, source_cfg in SOURCES.items():
            ...
            for thread in reversed(threads):
                ...
                elif thread["is_sticky"] and new_hash and known.get("hash") and new_hash != known["hash"]:
                    # Two-phase confirmation: only alert if we see the same new
                    # hash on two consecutive polls.  This protects against
                    # dynamic page content (timestamps, view counts, ads) that
                    # make the raw HTML differ on every scrape.
                    pending = known.get("pending_hash")
                    if pending == new_hash:
                        # Confirmed — hash stable across two polls
                        embed = self._build_embed(thread, source_cfg, post_data, is_update=True)
                        await self._send(channel, embed, ping_roles.get(source_key))
                        source_seen[tid]["hash"] = new_hash
                        source_seen[tid]["pending_hash"] = None
                        source_seen[tid]["pending_at"] = None
                        log.info("Updated sticky %s (%s) in guild %d", tid, source_key, guild_id)
                    else:
                        # First sighting of this hash — store as pending and wait
                        # for confirmation on the next poll cycle.
                        source_seen[tid]["pending_hash"] = new_hash
                        source_seen[tid]["pending_at"] = datetime.now(timezone.utc).isoformat()
                        log.debug("Pending update for sticky %s (%s): hash changed, awaiting confirmation", tid, source_key)
    ...
    async def _cmd_togglesource(self, ctx: fluxer.Message, args: str) -> None:
        ...
        if source not in SOURCES:
            ...
            return
        data = self._data(ctx.guild_id)
        was_enabled = data["enabled_sources"].get(source, True)
        data["enabled_sources"][source] = not was_enabled
        self._save(ctx.guild_id, data)
        state = "enabled" if data["enabled_sources"][source] else "disabled"
        await ctx.reply(content=f"✅ Source `{source}` is now **{state}**.")
        # If we just enabled a source that was previously disabled, seed its
        # current threads immediately so we don't spam existing posts.
        if not was_enabled and data["enabled_sources"][source]:
            await self._seed_guild(ctx.guild_id)
